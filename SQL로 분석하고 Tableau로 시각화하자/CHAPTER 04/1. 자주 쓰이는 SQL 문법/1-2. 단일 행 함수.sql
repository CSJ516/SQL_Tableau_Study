...
SELECT  GETDATE()   --YYYY-MM-DD HH:MI:SS
...
--STUFF: 문자열의 지정 위치 및 길이 만큼의 문자를 지정 문자열로 변환
...
SELECT  CONVERT(VARCHAR, GETDATE(), 5)   --MM-DD-YY
SELECT  CONVERT(VARCHAR, GETDATE(), 8)   --HH:MI:SS

SELECT  CONVERT(VARCHAR, GETDATE(), 21)   --YYYY-MM-DD HH:MI:SS
SELECT  CONVERT(VARCHAR, GETDATE(), 23)   --YYYY-MM-MM
...
SELECT  CAST('100' AS INT) + CAST('100' AS INT)       --200
SELECT  CAST(100 AS VARCHAR) + CAST(100 AS VARCHAR)   --100100

SELECT  CAST(1.56 AS INT)       --1
SELECT  CAST(1.56 AS NUMERIC)   --2

SELECT  CAST(GETDATE() AS DATE)       --YYYY-MM-DD
SELECT  CAST(GETDATE() AS DATETIME)   --YYYY-MM-DD HH:MI:SS
...
--NULLIF: 두 개의 특정 열이 같은 경우 NULL / 다를 경우 왼쪽 열 반환
SELECT  NULLIF(1, 1)
SELECT  NULLIF(1, 2)
