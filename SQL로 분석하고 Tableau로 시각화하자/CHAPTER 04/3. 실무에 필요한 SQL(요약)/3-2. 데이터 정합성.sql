...
--[MART_2020] 데이터 마트의 회원수 중복은 없는가?
SELECT  COUNT(mem_no) AS 회원수
		,COUNT(DISTINCT mem_no) AS 회원수_중복제거
  FROM  [MART_2020]

--[Member] 테이블과 [MART_2020] 데이터 마트의 회원수 차이는 없는가?
SELECT  COUNT(mem_no) AS 회원수
		,COUNT(DISTINCT mem_no) AS 회원수_중복제거
  FROM  [Member]

--[Order] 테이블과 [MART_2020] 데이터 마트의 주문수 차이는 없는가?
...
--[MART_2020] 데이터 마트의 미구매자는 [Order] 테이블에서 2020년에 구매가 없는가?
SELECT  *
  FROM  [Order]
